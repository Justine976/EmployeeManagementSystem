    ...
    private void resetDatabaseButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_resetDatabaseButtonActionPerformed
        if (JOptionPane.showConfirmDialog(this, "Are you sure you want to RESET The DATABASE? This Cannot be Undone", "Confirmation",
            ...
            try {
                ...
                // SQLite has no "DROP DATABASE" — drop the tables instead.
                // MainFrame.Connect() recreates the schema and re-seeds the
                // default admin, sample employees and their QR codes on restart.
                java.sql.Statement stmt = mainFrame.connection.createStatement();
                stmt.executeUpdate("DROP TABLE IF EXISTS payroll_table");
                stmt.executeUpdate("DROP TABLE IF EXISTS attendance_table");
                stmt.executeUpdate("DROP TABLE IF EXISTS employees_table");
